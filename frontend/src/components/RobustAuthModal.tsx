...
interface RobustAuthModalProps {
  children?: ReactNode;
  defaultTab?: 'login' | 'register' | 'forgot';
}

export function RobustAuthModal({ children, defaultTab = 'login' }: RobustAuthModalProps) {
  ...
  const handleOpen = () => {
    console.log('Opening robust modal...');
    setOpen(true);
    setMode(defaultTab);
  };

  const handleClose = () => {
    console.log('Closing robust modal...');
    setOpen(false);
    setMode(defaultTab);
